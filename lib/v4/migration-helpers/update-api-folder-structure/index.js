...
const updateConfig = require('../update-application-config');
...
const updateApiFolderStructure = async (appPath) => {
  ...
  try {
    // Copy the config folder to the v3 folder for safe keeping
    await fs.copy(configPath, join(strapiAppPath, 'v3', 'config'));
  ...
  for (const api of apiDirs) {
    ...
    await updateConfig(configPath, 'sqlite', liquidEngine);
