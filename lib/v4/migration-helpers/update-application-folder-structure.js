...
module.exports = async (appPath) => {
  ...
  try {
    ...
    // Load the templates
    const adminAppExample = await liquidEngine.renderFile(`src-app.example`);
    const adminWebpackExample = await liquidEngine.renderFile(`src-webpack.config.example`);
    ...
    logger.error(`an error occurred when creating admin example files`);
    console.log(error);
  }
