...
module.exports = async (apiPath, apiName) => {
  ...
  try {
    ...
    // Remove count
    const updatedRoutes = routes.routes.filter(
      (route) => !route.handler.includes('count') || !route.path.includes('count')
    );

    const renamedRoutes = updatedRoutes.routes.map(route => {
      existingPath = route.match(/\/[^/]*/)[0]
      // route.path = route.path.replace(existingPath, `/${apiName}`)
      route.handler = route.handler.replace(existingPath, `/${apiName}`)
    }) 

    // Transform objects to strings
    const routesToString = inspect(renamedRoutes, { depth: Infinity });
