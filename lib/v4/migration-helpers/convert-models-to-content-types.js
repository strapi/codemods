...
const convertModelToContentType = async (apiPath, contentTypeName) => {
  ...
  try {
    ...
    const fixedRelations = relations.map(([key, value]) => {
      if(value.via && value.collection && isSingular(value.via) && isPlural(value.collection)){
        //Many-To-One
      ...
      } else if(value.model && isSingular(value.model) && !value.via && !value.collection){
        //One-Way
        console.log('One-Way', key)
        value = getRelationObject("oneToOne", value.model, false)
      } else if(value.via && value.model && isPlural(value.via) && isPlural(value.model)){
        //One-To-Many
      ...
      } else if(value.collection && isSingular(value.collection) && !value.via && !value.model){
        //Many-Way
